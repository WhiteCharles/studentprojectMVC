...
namespace studentprojectAPI.Controllers
{
    ...
    public class RecordsController : ControllerBase
    {
        ...
        [HttpGet]
        public async Task<IActionResult> Get()  // getRecords
        {
            try
            {
                var results = await _recordRepository.GetAllRecordsAsync();
                RecordDTO[] DTOModels = _mapper.Map<RecordDTO[]>(results);
                return Ok(DTOModels);
            }
            catch (Exception)
            {
        ...
        [HttpGet("{Title}")]
        public async Task<ActionResult<RecordDTO>> Get(string Title)
        {
            try
            {
                var result = await _recordRepository.GetRecordAsync(Title);
            ...
            catch (Exception)
            {
                ...
                return this.StatusCode(StatusCodes.Status500InternalServerError, "Database not responding. Listing records failed.");
            }
        }

        [HttpGet("search")]
        public async Task<ActionResult<RecordDTO[]>> SearchByTitle(string searchTerm)
        {
            try
            {
                var results = await _recordRepository.GetAllRecordsByTitle(searchTerm);

                if (!results.Any()) return NotFound();

                return _mapper.Map<RecordDTO[]>(results);
            }
            catch (Exception)
            {
                return this.StatusCode(StatusCodes.Status500InternalServerError, "Record Search by Title failed.");
            }
        }

        [HttpGet("nameSearch")]
        public async Task<ActionResult<RecordDTO[]>> SearchByArtist(string nameSearch)
        {
            try
            {
                var results = await _recordRepository.GetAllRecordsByArtist(nameSearch);
            ...
            {
                return this.StatusCode(StatusCodes.Status500InternalServerError, "Record Search by Artist failed.");
            }
        }
